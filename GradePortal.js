module.exports = { run: async () => {
  ...
  const { parse } = importModule("HTMLParser");

  if(config.runsInApp){
    let a = new Alert()
    a.title = "asdasdasd"
    a.message = "Potato in Alaska"
    a.present()
  }
  ...
  if(args.widgetParameter == ""){
    ...
    let titleField = titleStack.addText(args.widgetParameter);
    ...
    let desc = widget.addText("Click on this widget to setup!");
        desc.minimumScaleFactor = 0.5;
        desc.textColor = Color.dynamic(Color.black(), Color.white());
        desc.font = Font.semiboldSystemFont(20);
    ...
    const updatedTime = widget.addText('Last updated:' + " " + last_updated.toLocaleString());
